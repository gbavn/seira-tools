...
- **`genPokemon` — Gerador de Pokémon**: ficha completa em BBcode, consumida pelo parser `seira-parser.js` do fórum (fora deste repo). Modos `normal` / `starter` / `ovo` / `selvagem` (starter e ovo travam nível/EXP/itens). Espécie, forma regional, gênero, habilidade, pokébola, movimentos (6 slots com tag normal/TM/EM/Especial — cada tag vira uma tag BBcode própria `[move]`/`[move-tm]`/`[move-em]`/`[move-es]`, sem sufixo no nome; botão "Carregar Últimos Movimentos" preenche os slots a partir de `moveset_by_level` do Pokémon, respeitando o nível atual), status treinados, estilos de concurso (Irado/Belo/Fofo/Sagaz/Forte — saem como atributos `cool`/`beautiful`/`cute`/`clever`/`tough` direto na tag `[poke]`, mesmas chaves de `game.character_pokemon`), OT, particularidades. A tag `[poke]` também carrega `num` (id/nº da dex, usado pelo parser no cabeçalho do card). Preview do card (`preview`/`previewHtml`, via `hooks/preview-factories.js` e `utils/poke-card-render.js`) é só uma aproximação local — quem renderiza o `[poke]` de fato é o `seira-parser.js` do fórum.
